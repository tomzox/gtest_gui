...
def PrintParagraph(astr, indent, bullet=False):
    ...
    for match in re.finditer(r'([STHIFCLBP])<("([\x00-\xff]*?)"|[^">][^>]*?)>', astr):
        if prev_end < match.start():
            ...
            helpTexts[-1] += "('''%s''', '%s'), " % (Escape(txt), "indent" if indent else "")
            rstOutput += txt
        prev_end = match.end()
        ...
        helpTexts[-1] += "('''%s''', %s), " % (Escape(txt), fmt)
...
while True:
    ...
    if line.startswith("=head1 "):
        ...
        if line.startswith("=head2 "):
            ...
            title = line.partition(" ")[2].strip()
            subIndex += 1
            helpSections.append((sectIndex, subIndex, title))
...
if not doRst:
    ...
    print("HELP_INDEX = [")
    for name in helpIndex:
        print("    '''%s'''," % name)
    print("]\n")

    print("HELP_SECTIONS = {")
    for section in helpSections:
        print("    (%d, %d): '''%s'''," % (section[0], section[1], section[2]))
    print("}\n")
