...
configured to use any other GUI application for opening trace snippets.
...
When core dumps are enabled in the kernel, the core will be save by GtestGui and
can be analyzed via the I<Extract stack trace from core dump> command in the
result log's context menu. When core dumps are not enabled, this option is
...
which means the exception causes the process to terminate.
...
characters, as these are assumed be be separators.
...
process and the second in the second process.
...
efficient to partition test cases by repetition than by sharding, or if a
combination of both is even better. In the mentioned example, it would not use
sharding, but instead run half the number of repetitions in one process, and
the second half in the second. For more complex configurations such as 10
repetitions of 9 test cases on 8 CPUs, a combination of both methods will be
used.
