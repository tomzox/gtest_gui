...
An executable file cannot be selected or "refreshed" while a test campaign is
running.
...
Any modification to the test case filter can be undone using the "Control-Z"
(while keyboard focus is in the entry field), or be redone using "Control-Y".
...
latter will not help as indicator if your test cases generate few or no trace
output.) The context menu offers opening the file containing complete trace
output read from the process, so you can check where the test case is stuck.
...
In principle GtestGui could work around this Gtest limitation by doing the
partitioning itself, but this would require passing lists of all test cases per
process down via the command line, which would result in a very long filter
string. On some platforms that string might exceed OS limitation for length of
command line parameter. Therefore this is not considered by GtestGui.
(Preferred solution would be enhancing Gtest framework to allow using an
external test case scheduler, connected via socket, that can dynamically pass
down sets of test cases to execute.)
...
following tests. It is not possible to disable the instable test automatically
and restart the test process after a crash as this would interfere with
partitioning by Gtest "sharding", i.e. the set of test cases run by that test
case would be altered. The only way around is for the user to stop the
campaign, manually disable the instable test case and then restart the test
campaign.
...
On the MS Windows platform, the configuration file is stored in sub-directory
named "GtestGui" below the platform-specific "Application Data" directory. (In
recent versions of Windows, this will be a hidden directory within the user
home directory.)

Note the configuration file is not removed when uninstalling the application
via pip. (The file is small though.)
...
If multiple instances of the GUI are started in the same working directory,
they will use the same directory for storage. For single-user systems this
works well enough, as conflicts may occur only when pressing "Run" button
concurrently in different instances. For multi-user setup it is not recommended
to share the trace directory. (Seeing other user's test results would be
confusing anyway.)
